...
from .. import config
from ..commands import get_obstacle_pos, get_obstacle_size
from .state import _root_height_env, _root_pos_env
...
def _box_surface_height_under_xy(env: ManagerBasedRLEnv, xy: torch.Tensor, xy_margin: float = 0.02) -> torch.Tensor:
    """
    ...
    above_footprint = torch.logical_and(dx <= half_size_x, dy <= half_size_y)

    obstacle_top_height = obstacle_pos[:, 2] + half_size_z

    return torch.where(above_footprint, obstacle_top_height, torch.full_like(obstacle_top_height, -torch.inf))
...
def _support_surface_height_under_base(
    ...
    ground_height = torch.full(
        (env.num_envs,), config.GROUND_HEIGHT, device=root_pos_env.device, dtype=root_pos_env.dtype
    )

    obstacle_height = _box_surface_height_under_xy(env, xy=root_pos_env[:, :2], xy_margin=0.02)
