...
class _RootChatterState:
    ...
    @classmethod
    def _from_env(
        cls, env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg
    ) -> "_RootChatterState":
    ...
    def _previous_from_env(
        ...
        return cls(
            root_z=runtime._get_or_init_env_buffer(
                env, f"{buffer_prefix}_root_z", current.root_z
            ),
            root_z_vel=runtime._get_or_init_env_buffer(
                env, f"{buffer_prefix}_root_z_vel", current.root_z_vel
            ),
    ...
    def _write_to_env(self, env: ManagerBasedRLEnv, *, buffer_prefix: str) -> None:
        ...
        runtime._set_env_buffer(
            env, f"{buffer_prefix}_projected_gravity_xy", self.projected_gravity_xy
        )
        runtime._set_env_buffer(
            env, f"{buffer_prefix}_roll_pitch_rate", self.roll_pitch_rate
        )
...
def root_chatter_l2(
    ...
    current = _RootChatterState._from_env(env, asset_cfg=asset_cfg)

    buffer_prefix = runtime._private_buffer_name("parkour_root_chatter", asset_cfg.name)

    previous = _RootChatterState._previous_from_env(
        env, buffer_prefix=buffer_prefix, current=current
    )

    vertical_penalty = _vertical_root_chatter_l2(
        current=current, previous=previous, chatter_cfg=chatter_cfg
    )
    ...
    just_reset = runtime._episode_start_mask(
        env, reference=penalty, grace_steps=chatter_cfg.reset_grace_steps
    )

    penalty = torch.where(just_reset, torch.zeros_like(penalty), penalty)

    current._write_to_env(env, buffer_prefix=buffer_prefix)
...
def _angular_root_chatter_l2(
    ...
    tilt_change = torch.linalg.norm(
        current.projected_gravity_xy - previous.projected_gravity_xy, dim=-1
    )
