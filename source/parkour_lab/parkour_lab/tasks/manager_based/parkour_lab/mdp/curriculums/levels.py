...
class ParkourStructureCfg:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.mesh_kwargs, Mapping) or not all(
            isinstance(name, str) for name in self.mesh_kwargs
        ):
...
class ParkourLevelCfg:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(
            isinstance(structure, ParkourStructureCfg) for structure in self.structures
        ):
            raise TypeError(
                f"{self.name}: structures must contain supported parkour structure configurations."
            )
...
def coerce_level_cfg(level: ParkourLevelCfg | Mapping[str, object]) -> ParkourLevelCfg:
    ...
    if not isinstance(name, str):
        raise TypeError(
            f"Curriculum level name must be a string, got {type(name).__name__}."
        )

    structures = level["structures"]
    if isinstance(structures, (str, bytes)):
        raise TypeError(
            "structures must be a sequence of parkour structure configurations."
        )

    try:
        normalized_structures = tuple(
            coerce_structure_cfg(structure)
            for structure in cast(Iterable[object], structures)
        )
...
def coerce_structure_cfg(
    ...
    if not isinstance(mesh_kwargs, Mapping):
        raise TypeError(
            f"mesh_kwargs must be a mapping, got {type(mesh_kwargs).__name__}."
        )
    ...
    return ParkourStructureCfg(
        ...
        position=_float_triplet(
            structure.get("position", (0.0, 0.0, 0.0)), field_name="structure position"
        ),
