...
def proprioceptive_procedural_configs(
    ...
    if acquisition_version == PROPRIO_STATIONARY_YAW_VERSION:
        try:
...
def recurrent_evaluation_source(checkpoint, physical_identity):
    ...
    if (
        ...
        != (
            PROPRIO_STATIONARY_YAW_CHANGE
            if protocol["version"] == PROPRIO_STATIONARY_YAW_VERSION
            else None
        )
...
def recurrent_training_main(args, parser):
    ...
    if refinement:
        ...
        if version == PROPRIO_STATIONARY_YAW_VERSION:
            protocol.update(
